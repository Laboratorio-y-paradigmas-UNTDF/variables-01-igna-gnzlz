...
export function swap<T>(arr: T[], i: number, j: number): void {
  const temp = arr[i]; // Lee el valor en arr[i] (R-value) y lo guarda en temp
  arr[i] = arr[j]; // Lee el valor en arr[j] (R-value) y lo asigna a arr[i] (L-value)
  arr[j] = temp; // Lee el valor en temp (R-value) y lo asigna a arr[j] (L-value)
}
...
export function doubleAll(arr: number[]): void {
  for (let i = 0; i < arr.length; i++) {
    arr[i] = arr[i] * 2; // Lee el valor actual (R-value), lo duplica y lo asigna de vuelta (L-value)
  }
}
...
export function findAndReplace<T>(arr: T[], oldVal: T, newVal: T): number {
  let count = 0;
  for (let i = 0; i < arr.length; i++) {
    // Comparo el valor actual (R-value) con oldVal. Si son iguales, asigno newVal (L-value) y aumento el contador.
    if (arr[i] === oldVal) {
      // El L-value es arr[i], que recibe la asignación de newVal. El R-value es arr[i] para la comparación.
      arr[i] = newVal;
      count++;
    }
  }
